...
(function() {
  ...
  DataUnit = (function(_super) {
    ...
    function DataUnit(header, data) {
      // Robust duck-typing for ArrayBuffer because instanceof fails across different JS contexts
      if (data && data.byteLength !== undefined && data.size === undefined) {
        this.buffer = data;
      } else {
        this.blob = data;
      }
  ...
  Image = (function(_super) {
    ...
    Image.prototype._getFrameAsync = function(buffers, callback, opts) {
      // Synchronous replacement for the old dynamic-Worker approach (broken in Vite/ESM).
      // The image may be split across multiple buffers (nBuffers=2 for Blob input), so
      // we process ALL buffers and concatenate their decoded pixel arrays.
      var parts = [];
      var totalPixels = 0;
      for (var b = 0; b < buffers.length; b++) {
        var arr = this._getFrame(buffers[b], this.bitpix, this.bzero, this.bscale);
        parts.push(arr);
        totalPixels += arr.length;
      }
      var pixels = new parts[0].constructor(totalPixels);
      var offset = 0;
      for (var p = 0; p < parts.length; p++) {
        pixels.set(parts[p], offset);
...
export const FITS = globalObj.astro.FITS;
